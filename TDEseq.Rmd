...
```
res=TDEseq(expData,metadata,LMM=FALSE)
```
or run TDEseq with Seurat Object directly, with group and stage information are included in seurat@meta.data.
```
res=TDEseq(seurat)
```
The differential expression results can be accessed as a data frame in the output object via res@dfTDEseqResults. 
...
```
res=TDEseq(expData,metadata,LMM=TRUE)
```
...
To perform TDEseq in Pseudo-cell mode,  we set `pseudocell` argument as 20, e.g. : 
```
group=metadata$group
res=TDEseq(expData,time,group=group,LMM=TRUE,pseudocell=20)
```

## 6. Apply TDEseq on real scRNA-seq data
All scRNA-seq data sets used in our originial paper are available on [here](https://zenodo.org/record/8271795). For more time series scRNA-seq data, please refer [TEDD database](http://tedd.obg.cuhk.edu.hk/frontend/dist/index.html#/)
...
**B. Parameter settings.** The web version of TDEseq provides identical parameter settings to the R package version of TDEseq. Users can select appropriate parameters according to the parameter description. 
