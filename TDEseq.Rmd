...
By default, TDEseq uses the Linear version. Alternatively, one can perform the mixed version of TDEseq, which accounts for individual level batch effects, such a batch structure could arise due to cells are derived from different individuals or handled seperately. This variable imposing the batch structure on the expression observations is called "group" in the meta data table. By accounting for batch effects in TDEseq, we turn on the `LMM` argument, e.g. : 
```
