...
```
res=TDEseq(X=expData,meta=metadata,LMM=FALSE)
```
or run TDEseq with Seurat Object directly, with group and stage information are included in seurat@meta.data, i.e. the seurat@meta.data should contain "group" column and "stage" column. An example seurat object file can be downloaded from [here](https://zenodo.org/record/8276740/files/scRNA-seq_liver_embryo_seuobj.rda).
```
...
```
res=TDEseq(X=expData,meta=metadata,LMM=TRUE)
```
...
```
res=TDEseq(X=expData,meta=meta,zmat=cov,LMM=TRUE)
```
...
```
p<-PatternHeatmap(res,features.show=c("Cse1l","Cdk4","Cbx5","Sardh","Birc5","G3bp1","Pah"))
print(p)
```
...
To perform TDEseq in Pseudo-cell mode,  we set `pseudocell` argument as 20. An example data to perform `pseudocell` mode is also available on [here](https://zenodo.org/record/8278207/files/scRNA-seq_COVID19_counts.rda). 
```
load("scRNA-seq_COVID19_counts.rda")
res=TDEseq(X=counts,meta=meta,LMM=TRUE,pseudocell=20)
```
